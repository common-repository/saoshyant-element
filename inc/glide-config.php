<?php
if ( !function_exists ( "sor_glide_list" )){
function sor_glide_list($global_key=false,$slide=false){
  ...
  $count=0;
	...
	foreach($slide as $slide_key => $slide_value):$count++;
		...
		sor_glide($global_key,$option,$layer);
...
if ( !function_exists ( "sor_glide" )){
function sor_glide($global_key=false,$slide=false,$layer=false,$first=false){

  
 		echo '<div class="sor-slide-item sor-animte-active sor-slide-'.sanitize_key($slide['key']).' " >';
 		echo '<div class="sor-slide-inner" >';
